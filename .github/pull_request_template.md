...
<!-- How was this verified? e.g. `npm run tauri dev` from apps/desktop, `npx tsc --noEmit`, CI run link. -->

## Checklist

- [ ] NatSpec docs added/updated (`@title`/`@notice`/`@dev`, plus `@param`/`@return`)
- [ ] `npx tsc --noEmit` and `cargo fmt --check` pass locally (from `apps/desktop[/src-tauri]`)
