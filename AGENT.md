...
Current state: **empty Tauri + React scaffold** in `lib/desktop/`, sidecar-ready
(shell plugin initialized, no binaries), blank frontend, borderless window.
...
renonce/
├── lib/
│   ├── README.md           # box overview + no-cross-import rule
...
│   └── sidecars/           # audit tool sources — convention only, see §7
├── .github/
...
## 7. Sidecars (audit tools)

- Tools (Slither et al.) ship as Tauri sidecars: standalone binaries spawned by
  Rust via `tauri-plugin-shell`, JSON over stdout. Convention + wiring
  checklist: `lib/sidecars/README.md`.
- Current state: plugin initialized in `run()`; **no** `bundle.externalBin`
  entries and **no** shell capability scopes — adding either without a real
  binary breaks the build / grants nothing. Add both with the first tool.
- No Python runtime is bundled — ship self-contained binaries only.

## 8. Window / custom titlebar
...
- Boxes never import across each other by relative path (see `lib/README.md`).
