...
Current state: **empty Tauri + React scaffold** — no audit features, blank
frontend, borderless window. All docs and comments are in **English**.

## 2. Tech stack

- Tauri 2 (Rust backend in `src-tauri/`, React 19 + TS + Vite frontend in `src/`)
- npm + Cargo. App identity: `productName "ReNonce"`, `com.renonce.app`
- `@tauri-apps/api` for frontend-to-Rust calls; `@tauri-apps/cli` via
...
│       └── (future: colocated component assets)
├── public/
│   └── assets/brand/       # full brand library + its own README.md
├── src-tauri/
│   ├── tauri.conf.json     # borderless window (JSON: no comments allowed)
│   ├── Cargo.toml
│   ├── build.rs            # tauri_build codegen (NatSpec //! header)
│   ├── capabilities/default.json  # permission sets (JSON: no comments)
...
`src-tauri/` as working dir for cargo commands.
...
- Library: `public/assets/brand/` — URL-ready at `/assets/brand/...`
  (Vite serves `public/` in dev and copies it to `dist/` on build).
  Variants, measured pixel sizes, format rules: `public/assets/brand/README.md`.
- Component imports: `src/assets/renonce-icon.svg`, `src/assets/renonce-lockup.svg`.
...
- New views go in dedicated components under `src/`; keep `App.tsx` thin.
